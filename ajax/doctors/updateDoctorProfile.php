...
else
{
    if(strlen($uname)>=5)
    {
        ...
        else
        {
            if (strlen($email)>=10) 
            {
                $updateAdmin=("update doctors set username='$uname',fname='$fname',lname='$lname',gender='$gender',nationality='$nationality',date_of_birth='$date_of_birth',email='$email',phone_no='$phoneNo' where id='$doctorId'");
